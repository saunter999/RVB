...
def sceqs(mu,delta,temp,doping):
	...
	for idx,ek in enumerate(e0k):
	    ...
	    Ek=( (ek-mu)**2+(J*dk[idx]*delta)**2 )**0.5
	    #gapksum+=tanhevl(0.5*Ek/temp)*delta*gk[idx]**2/Ek
	    gapksum+=tanhevl(0.5*Ek/temp)*delta*dk[idx]**2/Ek
	gapksum=gapksum/Ns
...
def scTceqs(mu,Tc,doping):
	...
	for idx,ek in enumerate(e0k):
	    Ek=abs(ek-mu)
	    sumk+=tanhevl(0.5*Ek/Tc)*dk[idx]**2/Ek
...
if __name__=="__main__":
	...
	Tm=linspace(1e-3,0.06,30)
	for dop in dopls:
	  print>>fgap,"#","doping=",dop
	  e0k=-2.0*t*dop*gk
	  mu=1.00;Delta=1.8;   ## Be careful to check that converged solution is not depending on inital values of parameters.
	  Nit=1000;err=1e-7
	  for Temp in Tm:
